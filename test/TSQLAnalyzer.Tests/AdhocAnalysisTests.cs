...
public class AdhocAnalysisTests
{
    ...
    [TestMethod]
    public void AdhocUpdateJoinUsingUnqualifiedTargetNameAndQualifiedFromAliasDoesNotRaiseSrd0018()
    {
        var result = Analyze(new AnalyzerOptions
        {
            Script = """
        ...
        Assert.IsFalse(
            result.Result!.Problems.Any(p => p.RuleId == "SqlServer.Rules.SRD0018"),
            "SRD0018 should not be raised when UPDATE target name is unqualified and FROM uses a qualified alias of the same table.");
        Assert.IsTrue(
            result.Result.Problems.Any(p => p.RuleId == "SqlServer.Rules.SRP0014"),
            "The ad-hoc batch should still be analyzed, so SRP0014 should remain reported for the table variable join.");
    }
