...
The extension spawns the `ErikEJ.TSQLAnalyzerCli` tool in its hidden long-lived
**server mode** (`--server-mode`) and communicates with it over newline-delimited
JSON on stdin/stdout. Document open/change/save events are debounced and the
current (possibly unsaved) buffer content is sent for analysis; the returned
problems are published as `vscode.Diagnostic` entries.
...
The analyzer executable must be available. Either:

- install the dotnet tool: `dotnet tool install --global ErikEJ.DacFX.TSQLAnalyzer.Cli`
  (exposes `tsqlanalyze` on the PATH), or
- point `tsqlAnalyzer.serverPath` at a build of `ErikEJ.TSQLAnalyzerCli`.
